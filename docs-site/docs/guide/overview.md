...
| 接收外部分享 | 从浏览器/编辑器经系统分享菜单把文本或图片发给 AI → [文档](/guide/share) |
| 语音输入 | 麦克风听写实时把语音转文字进输入框 → [文档](/guide/voice-input) |
| 运行模式（五种模式） | Build / Plan / Auto / Target / God 的权限控制与终止条件 → [文档](/guide/modes) |
